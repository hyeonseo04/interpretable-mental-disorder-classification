...
# -*- coding: utf-8 -*-
"""
classify_llm_direct_zeroshot.py — LLM 직접 분류 baseline (zero-shot, factor 미경유)

  세션 transcript(내담자 발화)를 LLM에 직접 입력해 우울/불안/중독/정상 중 하나로 분류.
  factor 방식(vllm_extract.py + 로지스틱 회귀)과의 성능 비교용 베이스라인.

  vllm_extract.py 와 동일한 데이터 처리 규약을 따른다:
    - session_id는 파일명 기준 + 정규화(_raw_→_check_, person 대문자)
    - 손상 JSON은 복구하지 않고 명시적으로 제외(*.corrupted.txt 기록)
    - person_id 대문자 통일
    - 출력에 session_id 포함 → 세 방식(factor/zero/few)이 동일 세션인지 대조 가능

출력: session_direct_zeroshot_{tag}.jsonl  (session_id, person_id, true, pred, parsed_ok, raw)
      + 세션/사람 단위 macro-F1, 혼동행렬 콘솔 출력

사용:
  CUDA_VISIBLE_DEVICES=0 uv run classify_llm_direct_zeroshot.py \
      ../data/Training/02.라벨링데이터 ../data/Validation/02.라벨링데이터 \
      --out result/session_direct_zeroshot_14b.jsonl \
...
def parse_class(text: str):
    """JSON에서 class를 파싱. 실패 시 본문 키워드 fallback, 그래도 없으면 None."""
    obj = {}
    ...
    if c in CLASS_SET:
        return c
    for cand in CLASSES:                 # fallback: 본문에서 키워드 탐색
...
def collect_jobs(splits, max_chars):
    """(session_id, person_id, true_class, transcript) 수집. 손상/발화없음 제외."""
...
def main():
    ...
    ap.add_argument("--out", default="result/session_direct_zeroshot.jsonl")
    ...
    outs = llm.chat([build_messages(tr) for *_, tr in jobs], sp)
    ...
    with open(args.out, "w", encoding="utf-8") as f:
        for (sid, pid, true_c, _), o in zip(jobs, outs):
            ...
            pred = parsed or "NORMAL"          # 파싱 실패 시 보수적 NORMAL
            if parsed is None:
                n_fail += 1
            y_sess.append(true_c); p_sess.append(pred)
    ...
    print(f"파싱 실패 {n_fail}/{len(jobs)}건 → NORMAL 처리")

    # ── 평가 ──
    print(f"\n[세션] macro-F1 {f1_score(y_sess, p_sess, average='macro'):.3f}"
          f" · acc {accuracy_score(y_sess, p_sess):.3f}")
    pids = list(person_pred)
    pp = [Counter(person_pred[p]).most_common(1)[0][0] for p in pids]   # 다수결
